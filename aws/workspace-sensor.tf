...
resource "restapi_object" "sensor" {
 ...
 resource "aws_cloudformation_stack" "nlb_stack" {
  name = "${var.baseName}-nlb-${terraform.workspace}"
  parameters = {
    baseName          = "${var.baseName}-nlb-${terraform.workspace}"
...
resource "aws_cloudformation_stack" "sensor_stack" {
  name = "${var.baseName}-sensor-${terraform.workspace}"
  parameters = {
    baseName          = "${var.baseName}-sensor-${terraform.workspace}"
    brainIP           = var.brainIP
